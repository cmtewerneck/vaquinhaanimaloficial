    ...
    public class AuthController : MainController
    {
        ...
        public async Task<ActionResult> RegistrarComConfirmacao(RegisterUserViewModel registerUser)
        {
            ...
            if (result.Succeeded)
            {
                ...
                //await _signInManager.SignInAsync(user, false);
                SendEmailUsuarioAdicionadoConfirmacao(user.Name, user.Email, tokenConfirmacao);
                return CustomResponse();
            }
            foreach (var error in result.Errors)
            {
        ...
        {
            var url = "http://localhost:4200/auth/reset-password-user/" + userMail + "/" + token;
